...
import "@oz-3.4.2-up/access/OwnableUpgradeable.sol";
...
import "@oz-3.4.2-up/token/ERC20/SafeERC20Upgradeable.sol";
...
contract StableJoeStaking is Initializable, OwnableUpgradeable {
    ...
    constructor(IERC20Upgradeable _joe) initializer {
        require(
            ...
            "StableJoeStaking: joe can't be address(0)"
        );
    ...
    function addRewardToken(IERC20Upgradeable _rewardToken) external onlyOwner {
        ...
        require(
            ...
            "StableJoeStaking: reward token can't be re-added"
        );
    ...
    function pendingReward(
        ...
        uint256 _rewardBalance = _token == joe
            ? _currRewardBalance.sub(_totalJoe)
            : _currRewardBalance;
    ...
    function sweep(IERC20Upgradeable _token, address _to) external onlyOwner {
        require(
            !isRewardToken[_token] && address(_token) != address(joe),
            "StableJoeStaking: token can't be swept"
        );
