...
import "@oz-4.7.3/proxy/Clones.sol";
import "@oz-4.7.3/utils/structs/EnumerableSet.sol";
...
contract LBFactory is PendingOwnable, ILBFactory {
    ...
    /// The max binStep set is 247. We use this method instead of an array to keep it ordered and to reduce gas
    mapping(IERC20 => mapping(IERC20 => bytes32))
    ...
    constructor(address _feeRecipient, uint256 _flashLoanFee) {
        if (_flashLoanFee > MAX_FEE)
            revert LBFactory__FlashLoanFeeAboveMax(_flashLoanFee, MAX_FEE);
        ...
        )
    {
        ...
        maxVolatilityAccumulated = _preset.decode(
            type(uint24).max,
            _shift += 16
        );
        ...
        returns (LBPairInformation[] memory LBPairsAvailable)
    {
        unchecked {
            ...
            bytes32 _avLBPairBinSteps = _availableLBPairBinSteps[_tokenA][
                _tokenB
            ];
            uint256 _nbAvailable = _avLBPairBinSteps.decode(
                type(uint8).max,
                248
            );
            ...
            if (_nbAvailable > 0) {
                ...
                for (uint256 i = MIN_BIN_STEP; i <= MAX_BIN_STEP; ++i) {
                    if (_avLBPairBinSteps.decode(1, i) == 1) {
                        LBPairInformation
                            memory _LBPairInformation = _LBPairsInfo[_tokenA][
                                _tokenB
                            ][i];
                        ...
                        LBPairsAvailable[_index] = LBPairInformation({
                            ...
                            ignoredForRouting: _LBPairInformation
                                .ignoredForRouting
                        });
    ...
    function setLBPairImplementation(
        address _LBPairImplementation
    ...
    function createLBPair(
        ...
        if (_LBPairImplementation == address(0))
            revert LBFactory__ImplementationNotSet();

        if (!_quoteAssetWhitelist.contains(address(_tokenY)))
            revert LBFactory__QuoteAssetNotWhitelisted(_tokenY);
        ...
        if (
            address(_LBPairsInfo[_tokenA][_tokenB][_binStep].LBPair) !=
            address(0)
        ...
        _LBPair = ILBPair(
            Clones.cloneDeterministic(_LBPairImplementation, _salt)
        );
...

        {
            ...
            _avLBPairBinSteps = bytes32(
                uint256(_avLBPairBinSteps) | (1 << _binStep)
            );

            // Increase the number of lb pairs by 1
            _avLBPairBinSteps = bytes32(
                uint256(_avLBPairBinSteps) + (1 << 248)
            );
    ...
    function setLBPairIgnored(
        ...
        LBPairInformation memory _LBPairInformation = _LBPairsInfo[_tokenA][
            _tokenB
        ][_binStep];
        if (address(_LBPairInformation.LBPair) == address(0))
            revert LBFactory__AddressZero();

        if (_LBPairInformation.ignoredForRouting == _ignored)
    ...
    function setPreset(
        ...
        bytes32 _preset = bytes32(
            (uint256(_packedFeeParameters) & type(uint144).max) |
                (uint256(_sampleLifetime) << 240)
        );
    ...
    function removePreset(uint16 _binStep) external override onlyOwner {
        if (_presets[_binStep] == bytes32(0))
            revert LBFactory__BinStepHasNoPreset(_binStep);
    ...
    function setFeesParametersOnPair(
        ...
        if (address(_LBPair) == address(0))
            revert LBFactory__LBPairNotCreated(_tokenX, _tokenY, _binStep);
    ...
    function setFlashLoanFee(
        ...
        if (_oldFlashLoanFee == _flashLoanFee)
            revert LBFactory__SameFlashLoanFee(_flashLoanFee);
        if (_flashLoanFee > MAX_FEE)
            revert LBFactory__FlashLoanFeeAboveMax(_flashLoanFee, MAX_FEE);
    ...
    function removeQuoteAsset(IERC20 _quoteAsset) external override onlyOwner {
        if (!_quoteAssetWhitelist.remove(address(_quoteAsset)))
            revert LBFactory__QuoteAssetNotWhitelisted(_quoteAsset);
    ...
    function _setFeeRecipient(address _feeRecipient) internal {
        ...
        if (_oldFeeRecipient == _feeRecipient)
    ...
    function _getPackedFeeParameters(
        ...
        if (_reductionFactor > Constants.BASIS_POINT_MAX)
            revert LBFactory__ReductionFactorOverflows(
                _reductionFactor,
                Constants.BASIS_POINT_MAX
            );

        if (_protocolShare > MAX_PROTOCOL_SHARE)
            revert LBFactory__ProtocolShareOverflows(
                _protocolShare,
                MAX_PROTOCOL_SHARE
            );

        {
            ...
            uint256 _maxVariableFee = (_prod * _prod * _variableFeeControl) /
                100;
    ...
    function _sortTokens(
        IERC20 _tokenA,
        IERC20 _tokenB
    ) private pure returns (IERC20, IERC20) {
