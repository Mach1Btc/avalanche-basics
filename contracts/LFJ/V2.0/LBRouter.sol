...
import "@oz-4.7.3/token/ERC20/IERC20.sol";
...
contract LBRouter is ILBRouter {
    ...
    modifier verifyInputs(
        ...
        if (
            _pairBinSteps.length == 0 ||
            _pairBinSteps.length + 1 != _tokenPath.length
    ...
    function getSwapIn(
        ...
        (
            uint256 _pairReserveX,
            uint256 _pairReserveY,
            uint256 _activeId
        ) = _LBPair.getReservesAndId();

        if (
            _amountOut == 0 ||
            (
                _swapForY
                    ? _amountOut > _pairReserveY
                    : _amountOut > _pairReserveX
            )
        )
            revert LBRouter__WrongAmounts(
                _amountOut,
                _swapForY ? _pairReserveY : _pairReserveX
        ...
        while (true) {
            ...
            if (_reserve != 0) {
                _amountOutOfBin = _amountOut >= _reserve
                    ? _reserve
                    : _amountOut;
                uint256 _amountInToBin = _swapForY
                    ? _amountOutOfBin.shiftDivRoundUp(
                        Constants.SCALE_OFFSET,
                        _price
                    )
                    : _price.mulShiftRoundUp(
                        _amountOutOfBin,
                        Constants.SCALE_OFFSET
                    );
                ...
                if (_amountInWithFees + _reserve > type(uint112).max)
                    revert LBRouter__SwapOverflows(_activeId);
            ...
            if (_amountOut != 0) {
                _activeId = _LBPair.findFirstNonEmptyBinId(
                    uint24(_activeId),
                    _swapForY
                );
            } else {
        ...
        while (true) {
            ...
            if (_bin.reserveX != 0 || _bin.reserveY != 0) {
                ...
                if (_amountInToBin > type(uint112).max)
                    revert LBRouter__BinReserveOverflows(_activeId);
    ...
    function addLiquidity(
        LiquidityParameters calldata _liquidityParameters
    )
        external
        override
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        if (_liquidityParameters.tokenX != _LBPair.tokenX())
            revert LBRouter__WrongTokenOrder();

        _liquidityParameters.tokenX.safeTransferFrom(
            msg.sender,
            address(_LBPair),
            _liquidityParameters.amountX
        );
        _liquidityParameters.tokenY.safeTransferFrom(
            msg.sender,
            address(_LBPair),
            _liquidityParameters.amountY
        );

        (depositIds, liquidityMinted) = _addLiquidity(
        ...
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        ...
        if (
            _liquidityParameters.tokenX == wavax &&
            _liquidityParameters.amountX == msg.value
        ) {
            ...
            _liquidityParameters.tokenY == wavax &&
        ...
        returns (uint256 amountToken, uint256 amountAVAX)
    {
        ...
        {
            if (!_isAVAXTokenY) {
                (_amountTokenMin, _amountAVAXMin) = (
                    _amountAVAXMin,
                    _amountTokenMin
                );
            }
            ...
            (amountToken, amountAVAX) = _isAVAXTokenY
                ? (_amountX, _amountY)
                : (_amountY, _amountX);
        }
        ...
        returns (uint256 amountOut)
    {
        ...
        if (_amountOutMin > amountOut)
            revert LBRouter__InsufficientAmountOut(_amountOutMin, amountOut);
    }
        ...
        returns (uint256 amountOut)
    {
        if (_tokenPath[_pairBinSteps.length] != IERC20(wavax))
            revert LBRouter__InvalidTokenPath(
                address(_tokenPath[_pairBinSteps.length])
            );
        ...
        if (_amountOutMinAVAX > amountOut)
    ...
    function swapExactAVAXForTokens(
        ...
        returns (uint256 amountOut)
    {
        if (_tokenPath[0] != IERC20(wavax))
            revert LBRouter__InvalidTokenPath(address(_tokenPath[0]));
        ...
        returns (uint256[] memory amountsIn)
    {
        ...
        if (amountsIn[0] > _amountInMax)
            revert LBRouter__MaxAmountInExceeded(_amountInMax, amountsIn[0]);
        ...
        if (_amountOutReal < _amountOut)
            revert LBRouter__InsufficientAmountOut(_amountOut, _amountOutReal);
    }
        ...
        returns (uint256[] memory amountsIn)
    {
        ...
        if (_amountOutReal < _amountAVAXOut)
            revert LBRouter__InsufficientAmountOut(
                _amountAVAXOut,
                _amountOutReal
            );
        ...
        returns (uint256[] memory amountsIn)
    {
        ...
        if (msg.value > amountsIn[0])
            _safeTransferAVAX(msg.sender, msg.value - amountsIn[0]);
    }
    ...
    function _addLiquidity(
        LiquidityParameters calldata _liq,
        ILBPair _LBPair
    )
        private
        ...
        returns (uint256[] memory depositIds, uint256[] memory liquidityMinted)
    {
        unchecked {
            ...
            if (
                _amountXAdded < _liq.amountXMin ||
                _amountYAdded < _liq.amountYMin
            )
    ...
    function _getAmountsIn(
        ...
        for (uint256 i = _pairs.length; i != 0; i--) {
            ...
            if (_binStep == 0) {
                ...
                amountsIn[i - 1] = amountOut_.getAmountIn(
                    _reserveIn,
                    _reserveOut
                );
            } else {
            ...
            for (uint256 i; i < _pairs.length; ++i) {
                ...
                if (_binStep == 0) {
                    ...
                    if (_token < _tokenNext) {
                        amountOut = amountOut.getAmountOut(
                            _reserve0,
                            _reserve1
                        );
            ...
            for (uint256 i; i < _pairs.length; ++i) {
                ...
                if (_binStep == 0) {
                    ...
                    if (_token < _tokenNext) {
                        ...
                        uint256 _amountOut = _amountIn.getAmountOut(
                            _reserve0,
                            _reserve1
                        );
                    ...
                    ILBPair(_pair).swap(
                        _tokenNext == ILBPair(_pair).tokenY(),
                        _recipient
                    );
                }
            }
    ...
    function _getLBPairInformation(
        ...
        ILBPair _LBPair = factory
            .getLBPairInformation(_tokenX, _tokenY, _binStep)
            .LBPair;
    ...
    function _getPair(
        ...
        if (_binStep == 0) {
            ...
            _pair = address(_getLBPairInformation(_tokenX, _tokenY, _binStep));
    }
