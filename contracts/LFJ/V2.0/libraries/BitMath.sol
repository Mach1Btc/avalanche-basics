...
library BitMath {
    ...
    function closestBit(
        ...
        return
            _rightSide
                ? closestBitRight(_integer, _bit - 1)
                : closestBitLeft(_integer, _bit + 1);
    }
    ...
    function significantBit(
        uint256 _integer,
        bool _isMostSignificant
    ) internal pure returns (uint8) {
        return
            _isMostSignificant
                ? mostSignificantBit(_integer)
                : leastSignificantBit(_integer);
    }
    ...
    function closestBitRight(
        ...
        unchecked {
            ...
            return
                (x == 0) ? type(uint256).max : mostSignificantBit(x) - _shift;
        }
    }
