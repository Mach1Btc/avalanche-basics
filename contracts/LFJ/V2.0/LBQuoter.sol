...
contract LBQuoter {
    ...
    function findBestPathFromAmountIn(
        ...
        for (uint256 i; i < swapLength; i++) {
            // Fetch swap for V1
            quote.pairs[i] = IJoeFactory(factoryV1).getPair(
                _route[i],
                _route[i + 1]
            );
            ...
            if (quote.pairs[i] != address(0) && quote.amounts[i] > 0) {
                ...
                if (reserveIn > 0 && reserveOut > 0) {
                    quote.amounts[i + 1] = JoeLibrary.getAmountOut(
                        quote.amounts[i],
                        reserveIn,
                        reserveOut
                    );
                    quote.virtualAmountsWithoutSlippage[i + 1] = JoeLibrary
                        .quote(
            ...
            // Fetch swaps for V2
            ILBFactory.LBPairInformation[] memory LBPairsAvailable = ILBFactory(
                factoryV2
            ).getAllLBPairs(IERC20(_route[i]), IERC20(_route[i + 1]));
                ...
                for (uint256 j; j < LBPairsAvailable.length; j++) {
                    if (!LBPairsAvailable[j].ignoredForRouting) {
                        bool swapForY = address(
                            LBPairsAvailable[j].LBPair.tokenY()
                        ) == _route[i + 1];
                        ...
                        returns (uint256 swapAmountOut, uint256 fees) {
                            if (swapAmountOut > quote.amounts[i + 1]) {
                                ...
                                (, , uint256 activeId) = LBPairsAvailable[j]
                                    .LBPair
                                    .getReservesAndId();
    ...
    function findBestPathFromAmountOut(
        ...
        for (uint256 i = swapLength; i > 0; i--) {
            // Fetch swap for V1
            quote.pairs[i - 1] = IJoeFactory(factoryV1).getPair(
                _route[i - 1],
                _route[i]
            );
            if (quote.pairs[i - 1] != address(0) && quote.amounts[i] > 0) {
                ...
                if (reserveIn > 0 && reserveOut > quote.amounts[i]) {
                    quote.amounts[i - 1] = JoeLibrary.getAmountIn(
                        quote.amounts[i],
                        reserveIn,
                        reserveOut
                    );
            ...
            ILBFactory.LBPairInformation[] memory LBPairsAvailable = ILBFactory(
                factoryV2
            ).getAllLBPairs(IERC20(_route[i - 1]), IERC20(_route[i]));
                    ...
                    if (!LBPairsAvailable[j].ignoredForRouting) {
                        ...
                        returns (uint256 swapAmountIn, uint256 fees) {
                            if (
                                ...
                                quote.binSteps[i - 1] = LBPairsAvailable[j]
                                    .binStep;
                                ...
                                quote.fees[i - 1] =
                                    (fees * 1e18) /
                                    quote.amounts[i - 1]; // fee percentage in amountIn
                            }
                        } catch {}
                    }
                }
    ...
    function _getV2Quote(
        ...
        if (_swapForY) {
            quote = BinHelper
                .getPriceFromId(_activeId, _binStep)
                .mulShiftRoundDown(_amount, Constants.SCALE_OFFSET);
        } else {
            quote = _amount.shiftDivRoundDown(
