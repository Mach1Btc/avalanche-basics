...
library Hooks {
    ...
    bytes32 internal constant BEFORE_FLASH_LOAN_FLAG =
        bytes32(uint256(1 << 162));
    bytes32 internal constant AFTER_FLASH_LOAN_FLAG =
        bytes32(uint256(1 << 163));
    ...
    function encode(
        ...
        if (parameters.beforeFlashLoan)
            hooksParameters |= BEFORE_FLASH_LOAN_FLAG;
        ...
        if (parameters.afterBatchTransferFrom)
            hooksParameters |= AFTER_TRANSFER_FLAG;
    }
    ...
    function decode(
    ...
    ) internal pure returns (Parameters memory parameters) {
        ...
        parameters.beforeFlashLoan =
            (hooksParameters & BEFORE_FLASH_LOAN_FLAG) != 0;
        parameters.afterFlashLoan =
            (hooksParameters & AFTER_FLASH_LOAN_FLAG) != 0;
        ...
        parameters.beforeBatchTransferFrom =
            (hooksParameters & BEFORE_TRANSFER_FLAG) != 0;
        parameters.afterBatchTransferFrom =
            (hooksParameters & AFTER_TRANSFER_FLAG) != 0;
    }
    ...
    function getHooks(
        bytes32 hooksParameters
    ) internal pure returns (address hooks) {
    ...
    function setHooks(
        ...
        return
            bytes32(bytes12(hooksParameters)) |
            bytes32(uint256(uint160(newHooks)));
    }
    ...
    function getFlags(
        bytes32 hooksParameters
    ) internal pure returns (bytes12 flags) {
    ...
    function beforeFlashLoan(
        ...
        if ((hooksParameters & BEFORE_FLASH_LOAN_FLAG) != 0) {
            _safeCall(
                hooksParameters,
                abi.encodeWithSelector(
                    ILBHooks.beforeFlashLoan.selector,
                    sender,
                    to,
                    amounts
                )
    ...
    function beforeBatchTransferFrom(
        ...
        if ((hooksParameters & BEFORE_TRANSFER_FLAG) != 0) {
            _safeCall(
                hooksParameters,
                abi.encodeWithSelector(
                    ILBHooks.beforeBatchTransferFrom.selector,
                    sender,
                    from,
                    to,
                    ids,
    ...
    function afterBatchTransferFrom(
        ...
        if ((hooksParameters & AFTER_TRANSFER_FLAG) != 0) {
            _safeCall(
                hooksParameters,
                abi.encodeWithSelector(
                    ILBHooks.afterBatchTransferFrom.selector,
                    sender,
                    from,
                    to,
                    ids,
        ...
        assembly {
            ...
            success := and(
                ...
                and(
                    ...
                    eq(shr(224, mload(0)), expectedSelector)
                )
            )
        }
