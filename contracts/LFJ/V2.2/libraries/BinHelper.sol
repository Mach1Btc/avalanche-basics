...
library BinHelper {
     ...
     */
    function getSharesAndEffectiveAmountsIn(
        bytes32 binReserves,
        bytes32 amountsIn,
        uint256 price,
        uint256 totalSupply
        ...
        if (binLiquidity == 0 || totalSupply == 0)
            return (userLiquidity.sqrt(), amountsIn);

        shares = userLiquidity.mulDivRoundDown(totalSupply, binLiquidity);
        uint256 effectiveLiquidity = shares.mulDivRoundUp(
            binLiquidity,
            totalSupply
        );
        ...
        if (
            getLiquidity(binReserves.add(amountsIn), price) >
        ...
        if (x > 0) {
            unchecked {
                ...
                if (liquidity / x != price)
                    revert BinHelper__LiquidityOverflow();
            }
        }
        if (y > 0) {
            unchecked {
    ...
    function getCompositionFees(
        ...
        (uint128 receivedAmountX, uint128 receivedAmountY) = getAmountOutOfBin(
            binReserves.add(amountsIn),
            shares,
            totalSupply + shares
        ).decode();

        if (receivedAmountX > amountX) {
            uint128 feeY = (amountY - receivedAmountY).getCompositionFee(
                parameters.getTotalFee(binStep)
            );

            fees = feeY.encodeSecond();
        } else if (receivedAmountY > amountY) {
            uint128 feeX = (amountX - receivedAmountX).getCompositionFee(
                parameters.getTotalFee(binStep)
            );
        ...
        )
    {
...

        {
            ...
            uint128 maxAmountIn = swapForY
                ? uint256(binReserveOut)
                    .shiftDivRoundUp(Constants.SCALE_OFFSET, price)
                    .safe128()
                : uint256(binReserveOut)
                    .mulShiftRoundUp(price, Constants.SCALE_OFFSET)
                    .safe128();
            ...
            if (amountIn128 >= maxAmountIn) {
                ...
                amountOut128 = swapForY
                    ? uint256(amountIn)
                        .mulShiftRoundDown(price, Constants.SCALE_OFFSET)
                        .safe128()
                    : uint256(amountIn)
                        .shiftDivRoundDown(Constants.SCALE_OFFSET, price)
                        .safe128();
            ...
            (amountsInWithFees, amountsOutOfBin, totalFees) = swapForY
                ? (
                    amountIn128.encodeFirst(),
                    amountOut128.encodeSecond(),
                    fee128.encodeFirst()
                )
                : (
                    amountIn128.encodeSecond(),
                    amountOut128.encodeFirst(),
                    fee128.encodeSecond()
                );
        }
        ...
        if (
            getLiquidity(
                binReserves.add(amountsInWithFees).sub(amountsOutOfBin),
                price
            ) > Constants.MAX_LIQUIDITY_PER_BIN
        ) {
            revert BinHelper__MaxLiquidityPerBinExceeded();
    ...
    function received(
        ...
        IERC20 tokenX,
        IERC20 tokenY
    ) internal view returns (bytes32 amounts) {
