...
library FeeHelper {
    ...
    modifier verifyProtocolShare(uint128 protocolShare) {
        if (protocolShare > Constants.MAX_PROTOCOL_SHARE)
            revert FeeHelper__ProtocolShareTooLarge();
        _;
    }
    ...
    function getFeeAmountFrom(
        ...
        uint128 totalFee
    ) internal pure verifyFee(totalFee) returns (uint128) {
        unchecked {
    ...
    function getFeeAmount(
        ...
        unchecked {
            ...
            return
                uint128(
                    (uint256(amount) * totalFee + denominator - 1) / denominator
                );
        }
    }
        ...
        unchecked {
            ...
            return
                uint128(
                    (uint256(amountWithFees) *
                        totalFee *
                        (uint256(totalFee) + Constants.PRECISION)) / denominator
                );
        }
    }
    ...
    ) internal pure verifyProtocolShare(protocolShare) returns (uint128) {
        unchecked {
            return
                uint128(
                    (uint256(feeAmount) * protocolShare) /
