...
contract LBQuoter {
     ...
     */
    function getLegacyFactoryV2()
        public
        view
        returns (address legacyFactoryV2)
    {
    ...
    function findBestPathFromAmountIn(
        ...
        for (uint256 i; i < swapLength; i++) {
            if (_factoryV2_2 != address(0)) {
                ...
                ILBFactory.LBPairInformation[]
                    memory LBPairsAvailable = ILBFactory(_factoryV2_2)
                ...
                if (LBPairsAvailable.length > 0 && quote.amounts[i] > 0) {
                    for (uint256 j; j < LBPairsAvailable.length; j++) {
                        if (!LBPairsAvailable[j].ignoredForRouting) {
                            ...
                            try
                                ILBRouter(_routerV2_2).getSwapOut(
                            ...
                            returns (
                                ...
                                if (
                                    amountInLeft == 0 &&
                                    swapAmountOut > quote.amounts[i + 1]
                                ) {
                                    quote.amounts[i + 1] = swapAmountOut;
                                    quote.pairs[i] = address(
                                        LBPairsAvailable[j].LBPair
                                    );
                                    quote.binSteps[i] = uint16(
                                        LBPairsAvailable[j].binStep
                                    );
                                    quote.versions[i] = ILBRouter.Version.V2_2;

                                    // Getting current price
                                    uint24 activeId = LBPairsAvailable[j]
                                        .LBPair
                                        .getActiveId();
                                    ...
                                    quote.fees[i] = ((uint256(fees) * 1e18) /
                                        quote.amounts[i]).safe128(); // fee percentage in amountIn
                                }
                            } catch {}
                        }
                    }
            ...
            if (_factoryV2_1 != address(0)) {
                ...
                ILBFactory.LBPairInformation[]
                    memory LBPairsAvailable = ILBFactory(_factoryV2_1)
                        .getAllLBPairs(IERC20(route[i]), IERC20(route[i + 1]));
                ...
                if (LBPairsAvailable.length > 0 && quote.amounts[i] > 0) {
                    for (uint256 j; j < LBPairsAvailable.length; j++) {
                        if (!LBPairsAvailable[j].ignoredForRouting) {
                            ...
                            try
                                ILBRouter(_routerV2_1).getSwapOut(
                                    LBPairsAvailable[j].LBPair,
                                    quote.amounts[i],
                                    swapForY
                                )
            ...
            if (_legacyFactoryV2 != address(0)) {
                // Fetch swap for V2
                ILBLegacyFactory.LBPairInformation[]
                    memory legacyLBPairsAvailable = ILBLegacyFactory(
                        _legacyFactoryV2
                    ).getAllLBPairs(IERC20(route[i]), IERC20(route[i + 1]));
                ...
                if (legacyLBPairsAvailable.length > 0 && quote.amounts[i] > 0) {
                    for (uint256 j; j < legacyLBPairsAvailable.length; j++) {
                        if (!legacyLBPairsAvailable[j].ignoredForRouting) {
                            bool swapForY = address(
                                legacyLBPairsAvailable[j].LBPair.tokenY()
                            ) == route[i + 1];
                            try
                            ...
                            returns (uint256 swapAmountOut, uint256 fees) {
                                if (swapAmountOut > quote.amounts[i + 1]) {
                                    ...
                                    quote.binSteps[i] = legacyLBPairsAvailable[
                                        j
                                    ].binStep;
            ...
            if (_factoryV1 != address(0)) {
                address pair = IJoeFactory(_factoryV1).getPair(
                    route[i],
                    route[i + 1]
                );
                ...
                if (pair != address(0) && quote.amounts[i] > 0) {
                    ...
                    if (reserveIn > 0 && reserveOut > 0) {
                        uint256 swapAmountOut = JoeLibrary.getAmountOut(
                            quote.amounts[i],
                            reserveIn,
                            reserveOut
                        );
                ...
                if (LBPairsAvailable.length > 0 && quote.amounts[i] > 0) {
                    for (uint256 j; j < LBPairsAvailable.length; j++) {
                        if (!LBPairsAvailable[j].ignoredForRouting) {
                            ...
                            try
                                ILBRouter(_routerV2_2).getSwapIn(
                                    LBPairsAvailable[j].LBPair,
                                    quote.amounts[i],
                                    swapForY
                                )
                            returns (
                                ...
                                if (
                                    amountOutLeft == 0 &&
                                    swapAmountIn != 0 &&
                                    (swapAmountIn < quote.amounts[i - 1] ||
                                        quote.amounts[i - 1] == 0)
                                ) {
                                    quote.amounts[i - 1] = swapAmountIn;
                                    quote.pairs[i - 1] = address(
                                        LBPairsAvailable[j].LBPair
                                    );
                                    quote.binSteps[i - 1] = uint16(
                                        LBPairsAvailable[j].binStep
                                    );
                                    quote.versions[i - 1] = ILBRouter
                                        .Version
                                        .V2_2;
                                    ...
                                    quote.fees[i - 1] = ((uint256(fees) *
                                        1e18) / quote.amounts[i - 1]).safe128(); // fee percentage in amountIn
                                }
                            } catch {}
                        }
                    }
                ...
                if (LBPairsAvailable.length > 0 && quote.amounts[i] > 0) {
                    for (uint256 j; j < LBPairsAvailable.length; j++) {
                        if (!LBPairsAvailable[j].ignoredForRouting) {
                            ...
                            try
                                ILBRouter(_routerV2_1).getSwapIn(
                                    LBPairsAvailable[j].LBPair,
                                    quote.amounts[i],
                                    swapForY
                                )
                        ...
                        if (!legacyLBPairsAvailable[j].ignoredForRouting) {
                            ...
                            returns (uint256 swapAmountIn, uint256 fees) {
                                if (
                                    ...
                                    quote.binSteps[
                                        i - 1
                                    ] = legacyLBPairsAvailable[j].binStep;
                                    quote.versions[i - 1] = ILBRouter
                                        .Version
                                        .V2;
                                    ...
                                    quote.fees[i - 1] = ((fees * 1e18) /
                                        quote.amounts[i - 1]).safe128(); // fee percentage in amountIn
                                }
                            } catch {}
                        }
                    }
            ...
            if (_factoryV1 != address(0)) {
                ...
                if (pair != address(0) && quote.amounts[i] > 0) {
                    ...
                    if (reserveIn > 0 && reserveOut > quote.amounts[i]) {
                        uint256 swapAmountIn = JoeLibrary.getAmountIn(
                            quote.amounts[i],
                            reserveIn,
                            reserveOut
                        );
        ...
        if (swapForY) {
            ...
            quote = amount
                .shiftDivRoundDown(
                    ...
                    PriceHelper.getPriceFromId(activeId, uint16(binStep))
                )
                .safe128();
        }
    }
