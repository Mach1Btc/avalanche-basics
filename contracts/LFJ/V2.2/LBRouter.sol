...
contract LBRouter is ILBRouter {
    ...
    modifier onlyFactoryOwner() {
        if (msg.sender != Ownable(address(_factory2_2)).owner())
            revert LBRouter__NotFactoryOwner();
        _;
    }
    ...
    {
        ILBPair _LBPair = ILBPair(
            _getLBPairInformation(tokenX, tokenY, binStep, Version.V2_2)
        );
    ...
    function _getLBPairInformation(
        ...
        if (version == Version.V2) {
            ...
            lbPair = address(
                _factory2_1.getLBPairInformation(tokenX, tokenY, binStep).LBPair
            );
        } else {
            lbPair = address(
                _factory2_2.getLBPairInformation(tokenX, tokenY, binStep).LBPair
            );
        }
