...
library PairParameterHelper {
    ...
    uint256 internal constant MASK_STATIC_PARAMETER =
     ...
     */
    function getBaseFactor(
        bytes32 params
    ) internal pure returns (uint16 baseFactor) {
    ...
    function getFilterPeriod(
        bytes32 params
    ) internal pure returns (uint16 filterPeriod) {
    ...
    function getDecayPeriod(
        bytes32 params
    ) internal pure returns (uint16 decayPeriod) {
    ...
    function getReductionFactor(
        bytes32 params
    ) internal pure returns (uint16 reductionFactor) {
     ...
     */
    function getVariableFeeControl(
        bytes32 params
    ) internal pure returns (uint24 variableFeeControl) {
     ...
     */
    function getProtocolShare(
        bytes32 params
    ) internal pure returns (uint16 protocolShare) {
     ...
     */
    function getMaxVolatilityAccumulator(
        bytes32 params
    ) internal pure returns (uint24 maxVolatilityAccumulator) {
     ...
     */
    function getVolatilityAccumulator(
        bytes32 params
    ) internal pure returns (uint24 volatilityAccumulator) {
     ...
     */
    function getVolatilityReference(
        bytes32 params
    ) internal pure returns (uint24 volatilityReference) {
    ...
    function getIdReference(
        bytes32 params
    ) internal pure returns (uint24 idReference) {
    ...
    function getTimeOfLastUpdate(
        bytes32 params
    ) internal pure returns (uint40 timeOflastUpdate) {
    ...
    function getOracleId(
        bytes32 params
    ) internal pure returns (uint16 oracleId) {
    ...
    function getActiveId(
        bytes32 params
    ) internal pure returns (uint24 activeId) {
    ...
    function getVariableFee(
        ...
        if (variableFeeControl != 0) {
            unchecked {
                ...
                uint256 prod = uint256(getVolatilityAccumulator(params)) *
                    binStep;
    ...
    function getTotalFee(
        ...
        unchecked {
            return
                (getBaseFee(params, binStep) + getVariableFee(params, binStep))
                    .safe128();
        }
    }
     ...
     * @return The updated encoded pair parameters
     */
    function setVolatilityReference(
        bytes32 params,
        uint24 volRef
    ) internal pure returns (bytes32) {
        if (volRef > Encoded.MASK_UINT20)
     ...
     * @return The updated encoded pair parameters
     */
    function setVolatilityAccumulator(
        bytes32 params,
        uint24 volAcc
    ) internal pure returns (bytes32) {
        if (volAcc > Encoded.MASK_UINT20)
            revert PairParametersHelper__InvalidParameter();
    ...
    function setStaticFeeParameters(
        ...
        if (
            ...
            decayPeriod > Encoded.MASK_UINT12 ||
            reductionFactor > Constants.BASIS_POINT_MAX ||
            protocolShare > Constants.MAX_PROTOCOL_SHARE ||
    ...
    function updateTimeOfLastUpdate(
        bytes32 params
    ) internal view returns (bytes32 newParams) {
    ...
    function updateVolatilityReference(
        ...
        unchecked {
            volRef = uint24(
                (volAcc * reductionFactor) / Constants.BASIS_POINT_MAX
            );
        }
    ...
    function updateVolatilityAccumulator(
        ...
        unchecked {
            deltaId = activeId > idReference
                ? activeId - idReference
                : idReference - activeId;
            volAcc = (uint256(getVolatilityReference(params)) +
                deltaId *
    ...
    function updateReferences(bytes32 params) internal view returns (bytes32) {
        ...
        if (dt >= getFilterPeriod(params)) {
            ...
            params = dt < getDecayPeriod(params)
                ? updateVolatilityReference(params)
                : setVolatilityReference(params, 0);
        }
