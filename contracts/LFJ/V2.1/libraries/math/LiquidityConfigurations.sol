...
library LiquidityConfigurations {
        ...
        returns (uint64 distributionX, uint64 distributionY, uint24 id)
    {
        ...
        if (
            uint256(config) > type(uint152).max ||
     ...
     * @return id The id of the bin to add the liquidity to
     */
    function getAmountsAndId(
        bytes32 config,
        bytes32 amountsIn
    ) internal pure returns (bytes32, uint24) {
        (uint64 distributionX, uint64 distributionY, uint24 id) = decodeParams(
            config
        );
