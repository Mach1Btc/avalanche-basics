...
contract CultTreasury is Ownable {
    ...
    constructor(address _cult, address _USDC, uint32 _secondsNeededForQueue) {
    ...
    function deposit(
        ...
        IERC20Mintable(cult).mint(msg.sender, payout);

        totalReserves = totalReserves.add(payout);
        emit ReservesUpdated(totalReserves);

        emit Deposit(_token, _amount, payout);
    }
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        // IOHMERC20(cult).burnFrom(msg.sender, value);
    ...
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        IOHMERC20(cult).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, cult, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(cult).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(cult).totalSupply().sub(totalDebt));
    }
    ...
    function valueOf(
        ...
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10 ** IERC20(cult).decimals()).div(
