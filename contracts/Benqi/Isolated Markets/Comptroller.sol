    ...
    ExponentialNoError
{
    ...
    /// @notice Emitted when supply cap for a qiToken is changed
    event NewSupplyCap(QiToken indexed qiToken, uint newSupplyCap);
    ...
    function mintAllowed(
        ...
        uint supplyCap = supplyCaps[qiToken];
        // Supply cap of 0 corresponds to unlimited supplying
        if (supplyCap != 0) {
            uint totalCash = QiToken(qiToken).getCash();
            uint totalBorrows = QiToken(qiToken).totalBorrows();
            uint totalReserves = QiToken(qiToken).totalReserves();
    ...
    function borrowAllowed(
        ...
        // Keep the flywheel moving
        updateAndDistributeBorrowerRewardsForToken(qiToken, borrower);
    ...
    function liquidateBorrowAllowed(
        ...
        if (!markets[qiTokenCollateral].accountMembership[borrower]) {
        ...
        if (isDeprecated(QiToken(qiTokenBorrowed))) {
            require(
     ...
     *  Note that `qiTokenCollateral` is the number of qiTokens the account owns in the market,
     *  whereas `borrowBalance` is the amount of underlying that the account has borrowed.
     */
    struct AccountLiquidityLocalVars {
        uint sumCollateral;
        uint sumBorrowPlusEffects;
        uint qiTokenCollateral;
        uint borrowBalance;
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // sumCollateral += tokensToDenom * qiTokenCollateral
    ...
    function _supportMarket(QiToken qiToken) external returns (uint) {
        ...
        require(qiToken.isQiToken(), "Must be an QiToken"); // Sanity check to make sure its really a QiToken

        Market storage newMarket = markets[address(qiToken)];
    ...
    /**
     * @notice Set the given supply caps for the given qiToken markets. Supplying that brings total supplies to or above supply cap will revert.
     * @dev Admin or supplyCapGuardian function to set the supply caps. A supply cap of 0 corresponds to unlimited supplying.
     * @param qiTokens The addresses of the markets (tokens) to change the supply caps for
    ...
    function _setMarketSupplyCaps(
        ...
        for (uint i = 0; i < numMarkets; i++) {
            supplyCaps[address(qiTokens[i])] = newSupplyCaps[i];
            emit NewSupplyCap(qiTokens[i], newSupplyCaps[i]);
        }
    }
    ...
    function _rescueFunds(address _tokenAddress, uint _amount) external {
        ...
        // Similar to qiTokens, if this is uint.max that means "transfer everything"
     ...
     * @param qiToken The market to synchronize indexes for
     ...
     * @param holder The address to claim rewards for
     * @param qiTokens The list of markets to claim rewards in
     */
    function claimReward(address holder, QiToken[] memory qiTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimReward(holders, qiTokens, true, true);
    }

    /**
     * @notice Claim all rewards for a specified group of users, tokens, and market sides
     * @param holders The addresses to claim for
     * @param qiTokens The list of markets to claim in
    ...
    function claimReward(
        ...
        for (uint i = 0; i < qiTokens.length; i++) {
            // Safety check that the supplied qiTokens are active/listed
            ...
            if (suppliers == true) {
                rewardDistributor.updateMarketSupplyIndex(qiToken);
                for (
                    uint holderIndex = 0;
            ...
            if (borrowers == true) {
                rewardDistributor.updateMarketBorrowIndex(qiToken);
                for (
                    uint holderIndex = 0;
                    holderIndex < holders.length;
                    holderIndex++
    ...
    /**
     * @notice Returns true if the given qiToken market has been deprecated
     * @dev All borrows in a deprecated qiToken market can be immediately liquidated
     * @param qiToken The market to check if deprecated
     */
    function isDeprecated(QiToken qiToken) public view returns (bool) {
        return
            markets[address(qiToken)].collateralFactorMantissa == 0 &&
            borrowGuardianPaused[address(qiToken)] == true &&
            qiToken.reserveFactorMantissa() == 1e18;
    }
