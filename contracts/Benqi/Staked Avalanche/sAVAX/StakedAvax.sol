...
import "@oz-3.4.2-up/token/ERC20/IERC20Upgradeable.sol";
import "@oz-3.4.2-up/access/AccessControlUpgradeable.sol";
import "@oz-3.4.2-up/utils/ReentrancyGuardUpgradeable.sol";
import "@oz-3.4.2-up/utils/PausableUpgradeable.sol";
import "@oz-3.4.2-up/math/SafeMathUpgradeable.sol";
    ...
    StakedAvaxStorage
{
    ...
    event TotalPooledAvaxCapUpdated(
        uint oldTotalPooldAvaxCap,
        uint newTotalPooledAvaxCap
    );
    ...
    function _mintShares(
        ...
        require(
            totalPooledAvax.add(avaxAmount) <= totalPooledAvaxCap,
            "TOTAL_POOLED_AVAX_CAP_EXCEEDED"
        );
    ...
    function _dropExpiredExchangeRateEntries() internal {
        ...
        uint shiftCount = 0;
        uint expirationThreshold = block.timestamp.sub(redeemPeriod).sub(
            172800
        );
    ...
    function setTotalPooledAvaxCap(uint newTotalPooledAvaxCap) external {
        require(
            hasRole(ROLE_SET_TOTAL_POOLED_AVAX_CAP, msg.sender),
        ...
        emit TotalPooledAvaxCapUpdated(
            oldTotalPooledAvaxCap,
            newTotalPooledAvaxCap
        );
    }
     ...
     */
    function setProtocolRewardShareRecipient(
    ...
    function setProtocolRewardShare(uint newProtocolRewardShare) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE");
        require(
            newProtocolRewardShare <= 1e18,
            "PROTOCOL_REWARD_SHARE_TOO_LARGE"
        );

        if (newProtocolRewardShare != 0) {
            require(
                protocolRewardShareRecipient != address(0),
                "PROTOCOL_REWARD_SHARE_RECIPIENT_NOT_SET"
            );
        }
