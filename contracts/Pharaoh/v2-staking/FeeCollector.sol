...
import "@oz-4.9.0/token/ERC20/utils/SafeERC20.sol";
...
contract FeeCollector is Initializable, IFeeCollector {
    ...
    /// @inheritdoc IFeeCollector
    function collectProtocolFees(IClPool pool) external override {
        // get tokens
        ...
        if (gauge == address(0) || !isAlive) {
            gauge = _voter.customGaugeForPool(address(pool));
