...
import "../core/interfaces/IPangolinERC20.sol";
...
contract PangolinBridgeMigrationRouter {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "PangolinBridgeMigrationRouter: Unauthorized"
        );
        _;
    }
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(
            msg.sender != account,
            "PangolinBridgeMigrationRouter: You can't demote yourself"
        );
    ...
    function addMigrator(
        ...
        require(
            ...
            "PangolinBridgeMigrationRouter: tokenAddress 0 not supported"
        );
        require(
            migratorAddress != address(0),
            "PangolinBridgeMigrationRouter: migratorAddress 0 not supported"
        );
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1, ) = IPangolinPair(pairToken)
            .getReserves();
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IPangolinPair(liquidityPair).burn(
            address(this)
        );
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(
            pairA != address(0),
            "PangolinBridgeMigrationRouter: liquidityPairFrom address 0"
        );
        require(
            pairA != address(0),
            "PangolinBridgeMigrationRouter: liquidityPairTo address 0"
        );
        require(
            pairA != pairB,
            "PangolinBridgeMigrationRouter: Cant convert to the same liquidity pairs"
        );
        require(
            IPangolinPair(pairA).token0() == IPangolinPair(pairB).token0() ||
                IPangolinPair(pairA).token0() ==
                IPangolinPair(pairB).token1() ||
                IPangolinPair(pairA).token1() ==
                IPangolinPair(pairB).token0() ||
     ...
     */
    function _migrateToken(address tokenAddress, uint amount) internal {
        require(
            tokenAddress != address(0),
            "PangolinBridgeMigrationRouter: tokenAddress 0 not supported"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IPangolinPair(liquidityPairFrom).permit(
            msg.sender,
            address(this),
            amount,
            deadline,
    ...
    function _migrateLiquidity(
        ...
        if (
            IPangolinPair(liquidityPairFrom).token0() ==
            IPangolinPair(liquidityPairTo).token0() ||
            IPangolinPair(liquidityPairFrom).token0() ==
            IPangolinPair(liquidityPairTo).token1()
        ) {
        ...
        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(
            liquidityPairFrom,
            amount
        );
        {
        ...
        if (
            IPangolinPair(liquidityPairFrom).token0() !=
            IPangolinPair(liquidityPairTo).token0() &&
            IPangolinPair(liquidityPairFrom).token1() !=
            IPangolinPair(liquidityPairTo).token1()
        ) {
     ...
     */
    function _calculateRescueLiquidity(
        address pairAddress,
        uint amount
    ) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IPangolinPair(pairAddress)
            .getReserves();
    ...
    function calculateChargeBack(
        ...
        require(
            ...
            "PangolinBridgeMigrationRouter: liquidityPairFrom address 0 not supported"
        );
        require(
            liquidityPairTo != address(0),
            "PangolinBridgeMigrationRouter: liquidityPairTo address 0 not supported"
        );
        (uint amountIn0, uint amountIn1) = _calculateRescueLiquidity(
            liquidityPairFrom,
            amount
        );
        if (
        ...
        (uint112 reserve0, uint112 reserve1, ) = IPangolinPair(liquidityPairTo)
            .getReserves();
        uint quote0 = amountIn0;
        uint quote1 = PangolinLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
