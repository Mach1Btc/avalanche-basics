...
import "@oz-4.8.1/access/AccessControlEnumerable.sol";
...
abstract contract PangoChefFunding is AccessControlEnumerable, GenericErrors {
    ...
    function addReward(uint256 amount) external onlyRole(FUNDER_ROLE) {
        ...
        if (block.timestamp >= periodFinish) {
            ...
            unchecked {
                uint256 leftover = (periodFinish - block.timestamp) *
                    _rewardRate;
                assembly {
                    newRewardRate := div(
                        add(amount, leftover),
     ...
     */
    function _poolPendingRewards(
        PoolRewardInfo storage pool,
        bool increment
        ...
        unchecked {
            ...
            uint256 rewardPerWeightPayable = rewardPerWeight -
                pool.rewardPerWeightPaid;
            rewards =
                pool.stashedRewards +
     ...
     */
    function _updateRewardPerWeightPaid(
        PoolRewardInfo storage pool
    ...
    function _getRewardPerWeightIncrementation()
        private
        view
        returns (uint128 incrementation)
    {
        ...
        assembly {
            incrementation := div(
                mul(globalPendingRewards, WEIGHT_PRECISION),
                tmpTotalWeight
            )
        }
    }
