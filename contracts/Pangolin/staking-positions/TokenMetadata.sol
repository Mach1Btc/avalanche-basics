...
import "@oz-4.8.1/utils/Base64.sol";
import "@oz-4.8.1/utils/Strings.sol";
...
contract TokenMetadata is AccessControlEnumerable {
    ...
    function tokenURI(
        IPangolinStakingPositions pangolinStakingPositions,
        uint256 positionId
        ...
        uint256 apr = balance == 0
            ? 0
            : (pangolinStakingPositions.positionRewardRate(positionId) *
                365 days *
                100) / balance;
        uint256 earned = parseEther(
            pangolinStakingPositions.positionPendingRewards(positionId)
        );
    ...
    function getImageUri(
        uint256 balanceLevel,
        uint256 durationLevel
    ) private view returns (string memory) {
        return
            string(
                abi.encodePacked(
