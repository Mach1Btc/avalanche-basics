...
import "../periphery/interfaces/IWAVAX.sol";
import "../core/interfaces/IPangolinFactory.sol";
import "../core/interfaces/IPangolinPair.sol";
import "./interfaces/IRewarder.sol";

/** @dev We call this contract with try/catch to figure out if pair is an actual pair. */
contract SafeExternalCalls {
    function getReserveTokenAddresses(
        address pair
...
contract PangoChef is PangoChefFunding, ReentrancyGuard {
    ...
    function claim(
        uint256 poolId
    ) external notEntered returns (uint256 reward) {
    ...
    function _addLiquidity(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IPangolinPair(poolToken)
            .getReserves();
    ...
    function _incrementLock(
        ...
        if (lockedPools[harvestPoolId][msg.sender].length() > MAX_LOCK_COUNT)
            revert OutOfBounds();
    }
    ...
    function _decrementLock(User storage user, uint256 withdrawPoolId) private {
        ...
        for (
            uint256 lastIndex = lockedPools[withdrawPoolId][msg.sender]
                .length();
            lastIndex != 0;
            ...
            uint256 lockedPoolId = lockedPools[withdrawPoolId][msg.sender].at(
                lastIndex
            );
    ...
    function _initializePool(
        ...
        if (poolType == PoolType.ERC20_POOL) {
            ...
            // Gas griefing is not possible as only 63/64 of the gas is forwarded per EIP-150.
            try
                safeExternalCalls.getReserveTokenAddresses(tokenOrRecipient)
     ...
     */
    function _updateRewardSummations(
        uint256 poolId,
        Pool storage pool
     ...
     */
    function _snapshotRewardSummations(
        Pool storage pool,
        User storage user
    ) private {
                ...
                : user.stashedRewards +
                    ((((deltaRewardSummations.idealPosition -
                        (deltaRewardSummations.rewardPerValue *
                            userLastUpdate)) * user.valueVariables.balance) +
                        (deltaRewardSummations.rewardPerValue *
