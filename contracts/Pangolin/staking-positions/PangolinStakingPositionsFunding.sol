...
import "@oz-4.8.1/token/ERC20/IERC20.sol";
 ...
 */
abstract contract PangolinStakingPositionsFunding is
    ...
    GenericErrors
{
    ...
    function endPeriod() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        unchecked {
            ...
            uint256 leftover = (tmpPeriodFinish - block.timestamp) *
                _rewardRate;
    ...
    function addReward(uint256 amount) external onlyRole(FUNDER_ROLE) {
        ...
        if (lastUpdate >= periodFinish) {
            ...
            unchecked {
                uint256 leftover = (uint256(periodFinish) - lastUpdate) *
                    _rewardRate;
                assembly {
                    newRewardRate := div(
                        add(amount, leftover),
